...
const macAddress = new RegExp(
  "^((([a-f0-9]{2}:){5})|(([a-f0-9]{2}-){5}))[a-f0-9]{2}$",
  "i"
);
const phoneNr = new RegExp(
  "\\+\\d{1,4}?[-.\\s]?\\(?\\d{1,3}?\\)?[-.\\s]?\\d{1,4}[-.\\s]?\\d{1,4}[-.\\s]?\\d{1,9}",
  "i"
);
const browserUA = new RegExp(
  "(firefox|msie|chrome|safari)[/\\s]([\\d.]+)",
  "i"
);

const latitudeLongitude = new RegExp(
  "^((\\-?|\\+?)?\\d+(\\.\\d+)?)[,;]\\s*((\\-?|\\+?)?\\d+(\\.\\d+)?)$",
  "i"
);

const gender = new RegExp(
  "^(transgender|two-spirit|non[-\\s]?binary|cisgender|gender[-\\s]?fluid|trans.*)$",
  "i"
);
const ssn = new RegExp(
  "^(?!000|666)[0-8][0-9]{2}-(?!00)[0-9]{2}-(?!0000)[0-9]{4}$",
  "i"
);
const birthday = new RegExp(
  "^([0123]?[0-9][-/][0123]?[0-9][-/][0-9]{2,4})|([0-9]{2,4}[-./][0123]?[0-9][-/][0123]?[0-9])|((jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec).*?\\s\\d+.*?\\s((([19]{2})?([1-9]{2}))|((20)([01][0-5]))))$",
  "i"
);
const bloodType = new RegExp("^(a|b|ab|o)[+-]$");
const iban = new RegExp(
  "^(?:(?:it|sm)\\d{2}[a-z]\\d{22}|cy\\d{2}[a-z]\\d{23}|nl\\d{2}[a-z]{4}\\d{10}|lv\\d{2}[a-z]{4}\\d{13}|(?:bg|bh|gb|ie)\\d{2}[a-z]{4}\\d{14}|gi\\d{2}[a-z]{4}\\d{15}|ro\\d{2}[a-z]{4}\\d{16}|kw\\d{2}[a-z]{4}\\d{22}|mt\\d{2}[a-z]{4}\\d{23}|no\\d{13}|(?:dk|fi|gl|fo)\\d{16}|mk\\d{17}|(?:at|ee|kz|lu|xk)\\d{18}|(?:ba|hr|li|ch|cr)\\d{19}|(?:ge|de|lt|me|rs)\\d{20}|il\\d{21}|(?:ad|cz|es|md|sa)\\d{22}|pt\\d{23}|(?:be|is)\\d{24}|(?:fr|mr|mc)\\d{25}|(?:al|do|lb|pl)\\d{26}|(?:az|hu)\\d{27}|(?:gr|mu)\\d{28})$",
  "i"
);
const creditcard = new RegExp(
  "^(?:4[0-9]{12}(?:[0-9]{3})?|(?:5[1-5][0-9]{2}|222[1-9]|22[3-9][0-9]|2[3-6][0-9]{2}|27[01][0-9]|2720)[0-9]{12}|3[47][0-9]{13}|3(?:0[0-5]|[68][0-9])[0-9]{11}|6(?:011|5[0-9]{2})[0-9]{12}| (?:2131|1800|35\\d{3})\\d{11})$",
  "i"
);
const mturk = new RegExp("^a[a-zA-Z0-9]{12,15}$", "i");
